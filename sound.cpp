...
void GameSound::SetVoiceAttr(unsigned int pitch, long channel, u_long soundAddr, unsigned char key)
{
    ...
    voiceAttributes.volume.left = 0x3FFF;
    voiceAttributes.volume.right = 0x3FFF;
    ...
    voiceAttributes.note = (key << 8) | 0xBA;
...
void GameSound::PlaySFX(VAGsound *sound, unsigned char key)
{
    // SpuVoiceAttr  voiceAttributes;
    ...
    SetVoiceAttr(sound->pitch, sound->spu_channel, sound->spu_address, key);

    SpuSetKey(SpuOn, sound->spu_channel);
}

void GameSound::LoadMusic(u_long *file)
{
...
long GameSound::ProcessMusic()
{
    if (instance)
    {
        if (instance->mus_tick < 125)
        {
            if (instance->mus_tick == 0)
            {
                instance->PlaySFX(&instance->mus, rand() % (165 - 157 + 1) + 157);
            }
            instance->mus_tick++;
        }
        else
